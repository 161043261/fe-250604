...
if (existsSync(confPath)) {
  const conf2 = JSON.parse(readFileSync(confPath, { encoding: "utf-8" })) as Partial<DbConfig>;
  conf = { ...conf, ...conf2 };
} else {
...
function createGroupMembers(): void {
  const sql = `
-- 群聊成员表
